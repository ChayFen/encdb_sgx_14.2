...
void ecall_getCacheUsed(double *d)
{
    *d = aesCache->getCacheUsed();
}
...
size_t bsearch(int64_t *arr, size_t size, int64_t x)
{
...
void quickSort(int64_t *nums, int64_t l, int64_t r)
{
    if (l >= r)
        return;
    int64_t i = l, j = r;
...
size_t ecall_bsearch(int64_t *arr, size_t size, int64_t x)
{
    return bsearch(arr, size, x);
}

void ecall_qsort(int64_t *arr, size_t size)
{
    quickSort(arr, 0, size - 1);
}


void ecall_init(void *pTaskQueue)
{
    #ifdef ECALL_BATCH
    static taskpool::TaskPoolLoop taskPoolLoop((taskpool::TaskQueue *)pTaskQueue);
    taskPoolLoop.loop();
    #endif
}