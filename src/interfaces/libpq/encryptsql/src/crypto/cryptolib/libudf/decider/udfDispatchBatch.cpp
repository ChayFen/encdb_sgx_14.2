...
char *saheAdd(char *cipher1, char *cipher2, char *aescipher1, char *aescipher2)
{
    ...
    if (path == HARDWARE && ecall_sahe_add)
    {
        ...
        taskpool::Task task{aescipher1, aescipher2, ret, taskpool::OpType::SAHE_ADD, taskpool::TaskStatus::T_PENDING};
        taskpool::g_taskQueue.putTaskBlock(&task);
        while (task.status != taskpool::TaskStatus::T_PROCESSED)
        {
