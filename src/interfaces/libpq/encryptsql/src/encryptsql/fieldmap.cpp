...
void FieldMapper::setDefaultValue()
{
    ...
    if( root->operator[]("timestamp").isNull()) {
        root->operator[]("timestamp") = 0;
    }
...
void FieldMapper::init()
{
    ...
    if (fin.is_open())
    {
        ...
        if (fin.eof() || !parseFromStream(builder, fin, root.get(), &errs)) {
            std::cout << "warning from FieldMapper init(): "  << errs << std::endl;
            // std::cout << "read:\n" << fin.rdbuf() << endl;
            // exit(-1);
        }
...
void FieldMapper::save(string _path)
{
    ...
    if (!fout.is_open())
    {
        LogError("error from FieldMapper save(): can not open %s", _path.c_str());
        std::cout << "error from FieldMapper save(): can not open " << _path << std::endl;
        abort();
    }
    else
    {
